...
async function listarVeiculo(){
    ...
    let response = await
    fetch("https://go-wash-api.onrender.com/api/auth/vehicle",{
        method: "GET",
        headers:{
            'Content-Type' : 'application/json', //Json porque consegue se comunicar  com informações no estilo (lembrar sobre e pesquisar, possível questão de prova)
            'Authorization' : `Bearer ${token.access_token}`
...
async function exibirVeiculos(dados){
    let tbody = document.querySelector("#veiculoTabela tbody")
    tbody.innerHTML = " "; //limpa a tabela antes de adicionar os dados
    for(let i=0;i<dados.length;i++)
    {
        let tr = document.createElement('tr');
